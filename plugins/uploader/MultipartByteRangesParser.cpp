...
std::string MultipartByteRangesParser::MakeUnuploadRanges(std::vector<UpLoadRange>& rExceptRanges, const std::string& rFileName)
{
	std::string result = "";
...
void MultipartByteRangesParser::MultipartParserExecute(const char *pBuffer, size_t iSize, const std::string& rBoundary, FILE *f, UploadTaskImpl* pTaskImpl)
{
	if (rBoundary == "--")
	{
		if (pTaskImpl->_uploadRanges.size() == 1)
		{
			if (_iPrevToByte != pTaskImpl->_uploadRanges[0]._iToByte)
			{
				fwrite(&pTaskImpl->_uploadRanges[0]._iFromByte, sizeof(uint64_t), 1, f);
				fwrite(&pTaskImpl->_uploadRanges[0]._iToByte, sizeof(uint64_t), 1, f);
				_iPrevToByte = pTaskImpl->_uploadRanges[0]._iToByte;
			}
