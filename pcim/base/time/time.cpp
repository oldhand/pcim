...
int Time::timestamp_milli(){
	return 0;
}

int Time::timestamp(){
	//11644473600 = ((1970 - 1601) * 365 + 89) * 24 * 60 * 60
	return TimeDelta::FromMicroseconds(Now().ToInternalValue()).ToSeconds() - 11644473600 - timezoneoffset();
}
int Time::timezoneoffset(){
	TIME_ZONE_INFORMATION tmp;
	GetTimeZoneInformation(&tmp);
	return tmp.Bias / (-60) * 60 * 60;
}
