...
#include "plugins/session/nim_cpp_msglog.h"
#include "plugins/session/nim_msglog_helper.h"
...
void SessionBox::InvokeShowMsgs(bool first_show_msg)
{
	...
	nim::MsgLog::QueryMsgAsync(session_id_, session_type_, kMsgLogNumberShow, farst_msg_time_,nbase::Bind(&TalkCallback::OnQueryMsgCallback, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
}
...
void SessionBox::InvokeShowSpecifiedCountMsgs(unsigned count)
{
	...
	//nim::MsgLog::QueryMsgAsync(session_id_, session_type_, count, farst_msg_time_,nbase::Bind(&TalkCallback::OnQueryMsgCallback, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
}
...
void SessionBox::OnUserInfoChange(const std::list<Db::Profile> &uinfos)
{
	auto refresh_msglist = [this](const Db::Profile& info) //更新消息列表中消息气泡的头像和名称
	{
		...
		//{
		//	auto iter = team_member_info_list_.find(info.id);
		...
		//		if (bubble_item)
		//		{
		//			bool refresh_header = bubble_item->msg_.sender_accid_ == info.id;
		...
		//if (refresh_show_name)
		//	RefreshMsglistShowname(info.id);
	};
	...
	for (auto info : uinfos)
	{
		...
		else if (session_type_ == nim::kNIMSessionTypeTeam)
		{
			if (team_info_.GetType() == nim::kNIMTeamTypeAdvanced)
			{
				...
				{
					//更新群成员列表信息
					/*TeamItem* item = (TeamItem*)member_list_->FindSubControl(nbase::UTF8ToUTF16(info.id));
					if (item != NULL)
