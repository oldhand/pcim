...
#include "utils/utf8_file_util.h"
...
void HeadModifyForm::LoadCurrentHeadImageForPreview()
{
	...
	std::wstring img_path = PhotoService::GetInstance()->GetUserPhoto(info.profileid);
	if(!img_path.empty())
	{
		if (nbase::FilePathIsExist(img_path, false))
		{ 
			current_head_bitmap_.reset(HeadImageUtil::GetFormatBitmap(img_path));
		}
	}
}
