...
#include "../plugins/contact/nim_ui_contacts_list_manager.h"
...
namespace nim_comp
{
	...
	{
		std::string profile_id = nim_comp::LoginManager::GetInstance()->GetProfileId();
		std::wstring head_image = nim_comp::PhotoService::GetInstance()->GetUserPhoto(profile_id);
	...
	void MainFormEx::PopupTrayMenu(POINT point)
	{
		...
		pMenu->Init(xml, _T("xml"), point, nim_comp::CMenuWnd::RIGHT_TOP);
		//注册回调
