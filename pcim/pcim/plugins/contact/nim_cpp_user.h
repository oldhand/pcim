...
public:
	...
	typedef std::function<void(const std::list<Db::Profile>&)> UserNameCardChangedCallback;					/**< 用户名片变更通知回调模板 */
	typedef std::function<void(const std::list<Db::Profile>&)> GetUserNameCardCallback;						/**< 获取用户名片回调模板 */
	...
	*				400:非法参数
	*/
	static bool UpdateMyUserNameCard(const Db::Profile& namecard, const UpdateMyUserNameCardCallback& cb, const std::string& json_extension = "");
