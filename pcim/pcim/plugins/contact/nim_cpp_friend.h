...
public:
	...
	typedef std::function<void(NIMResCode res_code, const std::list<Db::Contact>& user_profile_list)> GetFriendsListCallback;	/**< 获取好友回调模板 */
	typedef std::function<void(const std::string& accid, const std::list<Db::Contact>& user_profile)> GetFriendProfileCallback;	/**< 获取好友信息回调模板 */
