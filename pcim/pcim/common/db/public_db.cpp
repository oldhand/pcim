...
PublicDB::PublicDB()
{
    ...
    if (!sqls_created)
    {
		kCreateDBSQLs.push_back("CREATE TABLE IF NOT EXISTS logindata(uid TEXT PRIMARY KEY, name TEXT, password TEXT, type INTEGER, status INTEGER, remember_user INTEGER, remember_psw INTEGER, autologin INTEGER)");
