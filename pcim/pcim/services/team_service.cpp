...
namespace nim_comp
{
	...
	void TeamService::InvokeRemoveTeamMemberList(const std::string& tid, const std::list<std::string>& uid_lsit)
	{
		assert(nbase::MessageLoop::current()->ToUIMessageLoop());

		if (std::find_if(uid_lsit.begin(), uid_lsit.end(), [](const std::string& uid){
