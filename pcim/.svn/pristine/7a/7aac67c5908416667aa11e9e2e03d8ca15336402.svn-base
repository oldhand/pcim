...
namespace nbase
{
	std::list<std::string> listIntToString(std::list<int> lists) {
		std::list<std::string> new_lists;
		std::transform(lists.begin(), lists.end(), std::back_inserter(new_lists), [](int x){ return std::to_string(x); });
		return new_lists;
	}
	std::string join(std::list<std::string> lists, std::string delimitor){
		std::string result;
	...
	std::string join(std::list<int> lists, std::string delimitor){
		std::string result;
		for (auto item = lists.begin(); item != lists.end(); item++) {
			if (item != lists.begin()) {
				result.append(delimitor);
