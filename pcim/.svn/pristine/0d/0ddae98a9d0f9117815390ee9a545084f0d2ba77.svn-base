...
#include "plugins/team/team_callback.h"
...
void TeamInfoBox::OnUserInfoChange(const std::list<Db::Profile>& uinfos)
{
	for (auto iter = uinfos.cbegin(); iter != uinfos.cend(); iter++)
	{
		ui::HBox* item = (ui::HBox*)tile_box_->FindSubControl(nbase::UTF8ToUTF16(iter->id));
		if (item)
		{
			//if (iter->ExistValue(nim::kUserNameCardKeyIconUrl))
			//	item->FindSubControl(L"head_image")->SetBkImage(PhotoService::GetInstance()->GetUserPhoto(iter->id));

			//// 如果没有设置群昵称，则显示用户的备注名或昵称。如果设置了群昵称，就不变。
			//ui::Label* show_name_label = (ui::Label*)item->FindSubControl(L"show_name");
			//if (iter->ExistValue(nim::kUserNameCardKeyName) && team_member_list_.at(iter->id).GetNick().empty())
			//	show_name_label->SetText(UserService::GetInstance()->GetUserName(iter->id));
		}
	}
}
