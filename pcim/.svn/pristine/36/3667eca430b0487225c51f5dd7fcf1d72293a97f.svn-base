...
#include "plugins/contact/nim_friend_helper.h"
...
typedef std::function<void(const std::list<Db::Profile>&)> OnUserInfoChangeCallback;
typedef std::function<void(const std::list<Db::Profile>&)> OnGetUserInfoCallback;
...
public:
	...
	* @return void	无返回值
	*/
	void InvokeUpdateMyInfo(const Db::Profile &new_info, const OnUpdateUserInfoCallback& cb);
	...
	*/
	...
	/**
	* 获取本地保存的所有用户信息
	* @return std::map<std::string, Db::Profile>& 用户信息列表
	*/
	const std::map<std::string, Db::Profile>& GetAllUserInfos();

	/**
	* 获取本地保存的所有好友信息
	* @return std::map<std::string, nim::FriendProfile>& 好友信息列表
	*/
	const std::map<std::string, Db::Contact>& GetAllFriendInfos();
	...
	*/
	bool GetUserInfo(const std::string &id, Db::Profile &info);
	...
	* @return void 无返回值
	*/
	void GetUserInfos(const std::list<std::string> &ids, std::list<Db::Profile>&uinfos);
	...
	* @return void 无返回值
	*/
	void OnUserInfoChangeBySDK(const std::list<Db::Profile> &uinfo_list);
	...
	* @return void 无返回值
	*/
	void OnUserInfoChange(const std::list<Db::Profile> &uinfo_list);
...
private:
	std::map<std::string, Db::Profile> all_user_; //好友+陌生人
	std::set<std::string> post_ids_; //已经发出请求的id们
	std::map<std::string, Db::Contact> friend_list_; //好友列表
