...
typedef std::function<void(const std::string&, const std::list<std::string>&)> OnTeamMemberListRemove;
...
public:
	...
	* 触发批量移除群成员的的回调
	* @param[in] tid 群组id
	* @param[in] uid_lsit 被移除的用户列表
	* @return void	无返回值
	*/
	void InvokeRemoveTeamMemberList(const std::string& tid, const std::list<std::string>& uid_lsit);
