...
LanguageSetting GlobalManager::m_objLanguageSetting = LanguageSetting();
...
void GlobalManager::Startup(const std::wstring& strResourcePath, const CreateControlCallback& callback, bool bAdaptDpi, const std::wstring& theme, const LanguageSetting& language)
{
	...
	m_objLanguageSetting = language;

	// 加载多语言文件，如果使用了资源压缩包则从内存中加载语言文件
	if (g_hzip) {
		HGLOBAL hGlobal = GetData(strResourcePath + language.m_strResource + L"\\gdstrings.ini");
		if (hGlobal) {
		...
		MutiLanSupport::GetInstance()->LoadStringTable(strResourcePath + language.m_strResource + L"\\gdstrings.ini");
	}
...
LanguageSetting GlobalManager::GetLanguageSetting()
{
	return m_objLanguageSetting;
}
...
TFontInfo* GlobalManager::GetTFontInfo(const std::wstring& strFontId)
{
	...
	if (strFindId.empty())
	{
		if (m_sDefaultFontId.empty())
		{
			m_sDefaultFontId = L"1";
		}
